...
for m in range(0, k):
    ...
    for i in range(0,number_of_evaluation):
        ...
        rem_ndcg_evaluation.append(evaluation.evaluate_all(s_rem, test_queries))
        full_ndcg_evaluation.append(evaluation.evaluate_all(s_full, test_queries))

# write the result to file
timestamp = datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S')
f = open("../../output/experiment2/" + timestamp + ".txt", "w")
f.write("k: %s" % str(k) + "\n")
f.write("d: %s" % str(d) + "\n")
f.write("rem_ndcg_evaluation: %s" % str(rem_ndcg_evaluation) + "\n")
f.write("full_ndcg_evaluation: %s" % str(full_ndcg_evaluation))
f.close()
